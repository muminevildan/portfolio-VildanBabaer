...
// PHP'den JS'e oturum durumunu aktar
$girisYapti = isset($_SESSION['ad_soyad']) ? 'true' : 'false';
?>
<!DOCTYPE html>
<html lang="tr">
<head>
    ...
    <title>Ürünler ve Hizmetler — Vildan Babaer</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .giris-modal-overlay.aktif { opacity: 1; visibility: visible; }
        .giris-modal {
        ...
        .giris-modal-overlay.aktif .giris-modal { transform: translateY(0) scale(1); }
        [data-theme="dark"] .giris-modal { background: #1e1e38; border: 1px solid var(--light-gray); }
        .giris-modal-icon {
            font-size: 3rem; margin-bottom: 1rem; display: block;
            animation: bounce-icon 1.2s ease infinite alternate;
        }
        @keyframes bounce-icon { from { transform: translateY(0); } to { transform: translateY(-6px); } }
        .giris-modal h3 { font-family: 'Cormorant Garamond', serif; font-size: 1.8rem; font-weight: 400; color: var(--charcoal); margin-bottom: 0.8rem; }
        .giris-modal p { color: var(--warm-gray); font-size: 0.95rem; line-height: 1.7; margin-bottom: 2rem; }
        .giris-modal-butonlar { display: flex; gap: 1rem; justify-content: center; }
        .modal-btn-giris {
            ...
            box-shadow: 0 8px 20px rgba(232, 165, 184, 0.4);
        }
        .modal-btn-giris:hover { transform: translateY(-2px); box-shadow: 0 12px 30px rgba(232, 165, 184, 0.6); }
        .modal-btn-iptal {
            padding: 0.8rem 2rem; border-radius: 50px;
        ...
        .modal-btn-iptal:hover { border-color: var(--dusty-rose); color: var(--rose-deep); }
        .modal-kapat {
            position: absolute; top: 1rem; right: 1.2rem;
            background: none; border: none; font-size: 1.5rem;
            color: var(--warm-gray); cursor: pointer; line-height: 1;
            transition: color 0.2s ease;
        }
        .modal-kapat:hover { color: var(--rose-deep); }
        ...
        .page-subtitle { font-size: 1.05rem; color: var(--warm-gray); max-width: 600px; margin: 0 auto; line-height: 2; }
        ...
        .filter-btn {
            padding: 0.7rem 1.8rem; background: white; border: 2px solid var(--light-gray);
            border-radius: 50px; font-family: 'Montserrat', sans-serif; font-size: 0.8rem;
            letter-spacing: 1px; text-transform: uppercase; color: var(--warm-gray);
            cursor: pointer; transition: all 0.4s ease;
        }
        .filter-btn:hover, .filter-btn.active {
            background: linear-gradient(135deg, var(--dusty-rose), var(--lavender-deep));
            border-color: transparent; color: white; transform: translateY(-3px);
            box-shadow: 0 10px 30px rgba(232, 165, 184, 0.3);
        }

        /* ── Ürün Grid ─────────────────────────────────────────────── */
        .urunler-section { padding: 2rem 10% 6rem; max-width: 1400px; margin: 0 auto; }
        .urunler-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 2.5rem; }
        ...
        .urun-overlay { position: absolute; top: 1rem; right: 1rem; opacity: 0; transform: translateY(-10px); transition: all 0.4s ease; }
        ...
        .urun-ad { font-family: 'Cormorant Garamond', serif; font-size: 1.5rem; font-weight: 400; color: var(--charcoal); margin-bottom: 0.5rem; line-height: 1.3; }
        ...
        .etiket {
            padding: 0.25rem 0.7rem; background: var(--cream); border: 1px solid var(--light-gray);
            border-radius: 15px; font-size: 0.7rem; color: var(--warm-gray); transition: all 0.3s ease;
        }
        ...
        .btn-sepet {
            padding: 0.7rem 1.5rem; background: linear-gradient(135deg, var(--dusty-rose), var(--lavender-deep));
        ...
        #urunAra:focus { border-color: var(--dusty-rose) !important; box-shadow: 0 0 10px rgba(245, 150, 177, 0.2); }
...
<body>
    ...
    <!-- Giriş Gerekli Modal -->
    <div class="giris-modal-overlay" id="girisModal">
        <div class="giris-modal">
            <button class="modal-kapat" id="modalKapat">×</button>
            <span class="giris-modal-icon">🛍️</span>
            <h3>Giriş Yapmanız Gerekiyor</h3>
            <p>Sepete eklemek ve satın almak için hesabınıza giriş yapın veya yeni bir hesap oluşturun.</p>
            <div class="giris-modal-butonlar">
                <a href="giris_kayit.php" class="modal-btn-giris">Giriş Yap / Kayıt Ol</a>
                <button class="modal-btn-iptal" id="modalIptal">Vazgeç</button>
            </div>
        </div>
    </div>
    ...
    <nav id="navbar">
        ...
        <div class="nav-right-cluster">
            <ul class="nav-links">
                ...
                <li><a href="urunler.php" class="active" target="_self">Ürünler</a></li>
            </ul>
        ...
        <ul class="nav-links mobile-only">
            <!-- ✕ Kapat butonu (index.php ile aynı) -->
            ...
            <?php else: ?>
                ...
                <!-- 2×2 ikon grid (index.php ile aynı) -->
    ...
    <section class="page-hero">
        <div>
            ...
            <h1 class="page-title">Hizmetler & <span>Ürünler</span></h1>
            <p class="page-subtitle">
                Dijital dünyada ihtiyaç duyduğunuz tüm hizmetler ve ürünler tek çatı altında.
                Favorilere ekleyin veya sepetinize atın.
            </p>
        </div>
    </section>
    ...
    <section class="filter-section">
        <div class="filter-buttons">
            ...
            <button class="filter-btn" data-filter="tasarim">Tasarım</button>
            <button class="filter-btn" data-filter="marka">Marka</button>
            <button class="filter-btn" data-filter="egitim">Eğitim</button>
        </div>
    </section>

    <section class="urunler-section">
        <div class="search-container" style="max-width:500px; margin:0 auto 40px; padding:0 15px;">
            <input type="text" id="urunAra" placeholder="Ürün ismine veya özelliğine göre ara..."
                   style="width:100%; padding:15px 25px; border:2px solid var(--dusty-rose); border-radius:30px; font-family:'Montserrat',sans-serif; font-size:1rem; background:var(--cream); color:var(--charcoal); outline:none; box-shadow:0 5px 15px rgba(245,150,177,0.2); transition:all 0.3s ease;">
        </div>
        <div class="urunler-grid" id="urunlerGrid"></div>
    </section>
    ...
    <script>
        ...
        const GIRIS_YAPTI = <?php echo $girisYapti; ?>;

        // ── Modal ──────────────────────────────────────────────────
        function modalAc() {
            document.getElementById('girisModal').classList.add('aktif');
            document.body.style.overflow = 'hidden';
        }
        function modalKapat() {
            document.getElementById('girisModal').classList.remove('aktif');
        ...
        document.getElementById('girisModal').addEventListener('click', function(e) {
            if (e.target === this) modalKapat();
        });

        // Sepete ekleme — giriş yoksa modal
        document.addEventListener('click', function(e) {
            const btn = e.target.closest('.btn-sepet');
            if (!btn) return;
            if (!GIRIS_YAPTI) { e.preventDefault(); e.stopImmediatePropagation(); modalAc(); }
        }, true);

        // Favoriye ekleme — giriş yoksa modal
        document.addEventListener('click', function(e) {
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            try {
                ...
                if (savedTheme === 'dark') { htmlEl.setAttribute('data-theme', 'dark'); themeToggle.textContent = '☾'; }
                ...
                const grid = document.getElementById('urunlerGrid');
                if (grid && UrunlerApp.urunler.length > 0) {
                    let html = '';
                    UrunlerApp.urunler.forEach(urun => { html += UrunlerApp.kartOlustur(urun); });
                    grid.innerHTML = html;
                }
                ...
                document.querySelectorAll('.filter-btn').forEach(btn => {
                    btn.addEventListener('click', () => {
                        ...
                        UrunlerApp.filtrele(btn.getAttribute('data-filter'));
                    });
                });
        ...
        // Arama
        const aramaInput = document.getElementById('urunAra');
        if (aramaInput) {
            aramaInput.addEventListener('input', (e) => {
                const arananKelime = e.target.value.toLowerCase();
                const grid = document.getElementById('urunlerGrid');
                if (grid && UrunlerApp.urunler) {
                    const filtre = UrunlerApp.urunler.filter(urun =>
                        urun.ad.toLowerCase().includes(arananKelime) ||
                        urun.aciklama.toLowerCase().includes(arananKelime)
                    );
                    let html = '';
                    filtre.forEach(urun => { html += UrunlerApp.kartOlustur(urun); });
                    grid.innerHTML = html || '<p style="text-align:center;width:100%;color:var(--warm-gray);">Aradığınız kriterlere uygun ürün bulunamadı.</p>';
                    UrunlerApp.badgeYukle();
                }
            });
        }
