...
<head>
    ...
    <title>İletişim — Vildan Babaer</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sparkle { position: fixed; width: 4px; height: 4px; background: var(--gold); border-radius: 50%; opacity: 0; animation: sparkle 4s infinite; pointer-events: none; z-index: -1; }
        ...
        /* Hamburger — orijinal ile birebir */
        ...
        /* Responsive — orijinal mesajlar.php ile birebir aynı */
        ...
        /* Sayfa İçeriği */
        section {
            padding: 8rem 10% 6rem; max-width: 1400px;
            margin: 0 auto; position: relative; flex: 1;
        }
        ...
        .contact-link {
            display: flex; align-items: center; gap: 1rem; color: var(--warm-gray);
            text-decoration: none; font-size: 1.1rem; transition: all 0.3s ease;
            padding: 0.8rem 1.5rem; border-radius: 50px; background: white;
        }
        [data-theme="dark"] .contact-link { background: rgba(40,40,60,0.9); }
        ...
        .social-link {
            width: 50px; height: 50px; border: 2px solid var(--light-gray); border-radius: 50%;
            display: flex; align-items: center; justify-content: center; color: var(--charcoal);
            text-decoration: none; font-weight: 500; font-size: 0.9rem; transition: all 0.4s ease;
            position: relative; overflow: hidden;
        }
        [data-theme="dark"] .social-link { background: rgba(40,40,60,0.9); }
        .social-link::before { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, var(--dusty-rose), var(--lavender-deep)); opacity: 0; transition: opacity 0.4s ease; }
        ...
        .form-status {
            padding: 1rem 1.5rem; border-radius: 15px; margin-bottom: 2rem;
            font-size: 0.9rem; display: none; align-items: center; gap: 0.8rem;
            animation: slideDown 0.5s ease; border-left: 4px solid;
        }
        .form-status.show { display: flex; }
        .form-status.success { background: var(--success-light); border-left-color: var(--success); color: #2E7D32; }
        .form-status.error { background: var(--error-light); border-left-color: var(--error); color: #C62828; }
        .form-status .status-icon { font-size: 1.2rem; flex-shrink: 0; }
        [data-theme="dark"] .form-status.success { color: #a5d6a7; }
        [data-theme="dark"] .form-status.error { color: #ef9a9a; }
        @keyframes slideDown { from { opacity: 0; transform: translateY(-20px); } to { opacity: 1; transform: translateY(0); } }

        .form-group { margin-bottom: 2rem; position: relative; }
        .form-group input, .form-group textarea, .form-group select {
            width: 100%; padding: 1rem 0; border: none; border-bottom: 2px solid var(--light-gray);
            font-family: 'Montserrat', sans-serif; font-size: 1rem;
            color: var(--charcoal); background: transparent; transition: all 0.3s ease;
        }
        ...
        .form-group label {
            position: absolute; left: 0; top: 1rem; color: var(--warm-gray);
            font-size: 0.9rem; pointer-events: none; transition: all 0.3s ease;
        }
        .form-group input:focus, .form-group textarea:focus, .form-group select:focus {
            outline: none; border-bottom-color: var(--dusty-rose); border-bottom-width: 3px;
        }
        .form-group input:focus + label, .form-group textarea:focus + label,
        .form-group select:focus + label, .form-group input:not(:placeholder-shown) + label,
        .form-group textarea:not(:placeholder-shown) + label, .form-group select:valid + label {
            top: -0.5rem; font-size: 0.75rem; color: var(--dusty-rose);
        }
        .form-group.error input, .form-group.error textarea, .form-group.error select { border-bottom-color: var(--error); }
        .form-group.error label { color: var(--error); }
        .form-group.success input, .form-group.success textarea, .form-group.success select { border-bottom-color: var(--success); }
        .error-message {
            color: var(--error); font-size: 0.8rem; margin-top: 0.5rem;
            display: none; align-items: center; gap: 0.4rem; animation: fadeIn 0.3s ease;
        }
        .form-group.error .error-message { display: flex; }
        @keyframes fadeIn { from { opacity: 0; transform: translateY(-5px); } to { opacity: 1; transform: translateY(0); } }
        .required-mark { color: var(--dusty-rose); margin-left: 2px; }
        ...
        .submit-btn {
            ...
            border-radius: 50px; transition: all 0.4s ease; width: 100%; position: relative; overflow: hidden;
        }
        ...
        @media (max-width: 480px) {
            ...
            .contact-link { font-size: 0.9rem; padding: 0.6rem 1rem; }
            .contact-form { padding: 1.5rem; border-radius: 15px; }
            .submit-btn { font-size: 0.85rem; padding: 1rem 2rem; }
        }
    </style>
    <base target="_blank">
</head>
<body>
    ...
    <!-- ═══════════════════════════════════════════════════
         NAVBAR — mesajlar.php ile birebir aynı yapı
    ════════════════════════════════════════════════════ -->
    <nav id="navbar">
        ...
        <div class="nav-right-cluster">
            ...
            <div class="nav-actions">
                <?php if(isset($_SESSION['ad_soyad'])): ?>
                    ...
                    <a href="iletisim.php" class="nav-icon-link active" title="İletişim" target="_self">
    ...
    <section id="contact">
        <div class="section-header">
            ...
            <h1 class="section-title">Birlikte Çalışalım</h1>
        </div>
        <div class="contact-wrapper">
            <div class="contact-info">
                ...
                <div class="contact-links">
                    <a href="mailto:muminevildanb@gmail.com" class="contact-link" target="_self">
                        <span>✉</span> muminevildanb@gmail.com
                    </a>
                    <a href="#" class="contact-link">
                        <span>◍</span> Ankara, Türkiye
                    </a>
                </div>
                <div class="social-row">
                    <a href="#" class="social-link" title="Instagram"><span>İg</span></a>
                    <a href="#" class="social-link" title="LinkedIn"><span>In</span></a>
                    <a href="#" class="social-link" title="Twitter/X"><span>X</span></a>
                </div>
            </div>
            ...
            <form class="contact-form" id="contactForm" novalidate>
                ...
                <div class="form-group" id="groupName">
                    <input type="text" id="name" name="ad_soyad" placeholder=" " required minlength="2" maxlength="50">
                    <label for="name">Adınız Soyadınız<span class="required-mark">*</span></label>
                    <div class="error-message" id="errorName"><span>⚠</span> Lütfen adınızı ve soyadınızı girin (en az 2 karakter).</div>
                </div>
                <div class="form-group" id="groupEmail">
                    <input type="email" id="email" name="email" placeholder=" " required>
                    <label for="email">E-posta Adresiniz<span class="required-mark">*</span></label>
                    <div class="error-message" id="errorEmail"><span>⚠</span> Lütfen geçerli bir e-posta adresi girin.</div>
                </div>
                <div class="form-group" id="groupSubject">
                    <select id="subject" name="konu" required>
                        <option value="" disabled selected></option>
                        <option value="Yeni Proje Teklifi">Yeni Proje Teklifi</option>
                        <option value="İş Birliği">İş Birliği</option>
                        <option value="Danışmanlık">Danışmanlık</option>
                        <option value="Diğer">Diğer</option>
                    </select>
                    <label for="subject">Konu Başlığı<span class="required-mark">*</span></label>
                    <div class="error-message" id="errorSubject"><span>⚠</span> Lütfen bir konu seçin.</div>
                </div>
                <div class="form-group" id="groupPhone">
                    <input type="tel" id="phone" name="telefon" placeholder=" " pattern="[0-9]{11}" maxlength="11">
                    <label for="phone">Telefon Numaranız (Opsiyonel)</label>
                    <div class="error-message" id="errorPhone"><span>⚠</span> 11 haneli telefon numarası girin.</div>
                </div>
                <div class="form-group" id="groupMessage">
                    <textarea id="message" name="mesaj" placeholder=" " required minlength="10" maxlength="1000"></textarea>
                    <label for="message">Projenizden Bahsedin<span class="required-mark">*</span></label>
                    <div class="error-message" id="errorMessage"><span>⚠</span> Lütfen mesajınızı girin (en az 10 karakter).</div>
                </div>
    ...
    <footer>
        <div class="quote-container">
            <p class="rotating-quote active">"Yaratmak, ruhun nefes almasıdır"</p>
            <p class="rotating-quote">"Tasarım, görünmeyeni görünür kılmaktır"</p>
            <p class="rotating-quote">"Detaylar, mükemmelliğin mimarıdır"</p>
            <p class="rotating-quote">"Kod şiir, tasarım duygudur"</p>
        </div>
    ...
    <script>
        const EMAILJS_SERVICE_ID  = 'service_xh4ohfq';
        const EMAILJS_TEMPLATE_ID = 'template_kon4vvy';
        const EMAILJS_PUBLIC_KEY  = 'eI2gcP_4zCzz_QCfB';
        ...
        const validators = {
            name:    v => ({ valid: v.trim().length >= 2 && /^[a-zA-ZğüşıöçĞÜŞİÖÇ\s]+$/.test(v) }),
            email:   v => ({ valid: /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v.trim()) }),
            subject: v => ({ valid: !!v }),
            phone:   v => ({ valid: !v.trim() || /^[0-9]{11}$/.test(v.replace(/\s/g,'')) }),
            message: v => ({ valid: v.trim().length >= 10 && v.trim().length <= 1000 })
        };

        function validateField(fn, value) {
            const key   = fn.charAt(0).toUpperCase() + fn.slice(1);
            const group = document.getElementById('group' + key);
            const ok    = validators[fn](value).valid;
            group.classList.toggle('error',  !ok);
            group.classList.toggle('success', ok);
            return ok;
        }

        ['name','email','subject','phone','message'].forEach(fn => {
            const f = document.getElementById(fn);
            f.addEventListener('blur',  () => validateField(fn, f.value));
            f.addEventListener('input', () => {
                const g = document.getElementById('group' + fn.charAt(0).toUpperCase() + fn.slice(1));
                if (g.classList.contains('error')) validateField(fn, f.value);
            });
        });
        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            let isValid = true;
            ['name','email','subject','phone','message'].forEach(fn => {
                if (!validateField(fn, document.getElementById(fn).value)) isValid = false;
            });
            if (!isValid) {
                showStatus('error','Lütfen formdaki hataları düzeltin ve tekrar deneyin.');
                const first = document.querySelector('.form-group.error input,.form-group.error textarea,.form-group.error select');
                if (first) first.focus();
                return;
            }
            submitBtn.disabled = true;
            submitBtn.classList.add('loading');
            showStatus('success','Mesajınız gönderiliyor... Lütfen bekleyin.');

            const tp = {
                ad_soyad : document.getElementById('name').value,
                email    : document.getElementById('email').value,
                konu     : document.getElementById('subject').value,
                telefon  : document.getElementById('phone').value || 'Belirtilmedi',
                mesaj    : document.getElementById('message').value
            };

            emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, tp)
                .then(function() {
                    const fd = { id: Date.now(), ...tp, tarih: new Date().toLocaleString('tr-TR',{day:'2-digit',month:'long',year:'numeric',hour:'2-digit',minute:'2-digit'}) };
                    let msgs = JSON.parse(localStorage.getItem('contactMessages') || '[]');
                    msgs.unshift(fd);
                    localStorage.setItem('contactMessages', JSON.stringify(msgs));
                    showStatus('success','✓ Mesajınız başarıyla gönderildi! En kısa sürede size dönüş yapacağım.');
                    contactForm.reset();
                    ...
                    showStatus('error','✕ Mesaj gönderilemedi. Lütfen daha sonra tekrar deneyin.');
                    submitBtn.disabled = false;
                    submitBtn.classList.remove('loading');
                });
        });
        ...
        function showStatus(type, msg) {
            ...
            if (type === 'success') formStatus.scrollIntoView({ behavior:'smooth', block:'nearest' });
        }
        function hideStatus() { formStatus.classList.remove('show'); }
        ...
        // Visitor Counter
        (function() {
            let c = localStorage.getItem('visitorCount');
            c = c ? parseInt(c) + 1 : Math.floor(Math.random() * 1000) + 500;
        ...
        function createPetal() {
            ...
            p.style.color = ['var(--dusty-rose)','var(--lavender-deep)','var(--peach-deep)','var(--sage)'][Math.floor(Math.random()*4)];
        ...
        let cq = 0;
        setInterval(() => {
            quotes[cq].classList.remove('active');
            cq = (cq + 1) % quotes.length;
            quotes[cq].classList.add('active');
        }, 5000);
    </script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            if (typeof UrunlerApp !== 'undefined') {
                ...
                if (t) t.addEventListener('click', () => { if (typeof UrunlerApp !== 'undefined') UrunlerApp.temaToggle(); });
            }
        });
    </script>
</body>
