...
<head>
    ...
    <title>Sepetim — Vildan Babaer</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .page-hero {
            ...
            padding: 10rem 10% 2rem; text-align: center;
        }
        .page-greeting {
        ...
        /* Sepet */
        .sepet-section { padding: 2rem 10% 6rem; max-width: 1000px; margin: 0 auto; }

        .sepet-item {
            display: grid; grid-template-columns: 100px 1fr auto auto auto;
            gap: 1.5rem; align-items: center; padding: 1.5rem;
            background: white; border-radius: 20px; margin-bottom: 1rem;
            box-shadow: 0 10px 40px rgba(0,0,0,0.06);
            transition: all 0.3s ease; animation: slideUp 0.5s ease forwards;
            opacity: 0;
        }
        @keyframes slideUp {
        ...
        .sepet-item:hover { transform: translateY(-3px); box-shadow: 0 15px 50px rgba(0,0,0,0.1); }
        [data-theme="dark"] .sepet-item { background: rgba(40, 40, 60, 0.9); border: 1px solid var(--light-gray); }

        .sepet-resim { width: 100px; height: 80px; object-fit: cover; border-radius: 12px; }
        .sepet-detay h4 {
            font-family: 'Cormorant Garamond', serif; font-size: 1.3rem;
            font-weight: 400; color: var(--charcoal); margin-bottom: 0.3rem;
        }
        .sepet-fiyat { color: var(--warm-gray); font-size: 0.85rem; }
        .sepet-adet-kontrol {
            display: flex; align-items: center; gap: 0.8rem;
            background: var(--cream); border-radius: 50px; padding: 0.3rem;
        }
        [data-theme="dark"] .sepet-adet-kontrol { background: rgba(255,255,255,0.05); }
        .btn-adet {
            width: 35px; height: 35px; border-radius: 50%;
            background: white; border: 1px solid var(--light-gray);
            cursor: pointer; font-size: 1.2rem; color: var(--charcoal);
            display: flex; align-items: center; justify-content: center;
        ...
        .adet-sayi { font-weight: 500; min-width: 20px; text-align: center; color: var(--charcoal); }
        .sepet-toplam {
        ...
        .btn-sil {
            width: 40px; height: 40px; border-radius: 50%;
            background: var(--soft-pink); border: none; cursor: pointer;
            font-size: 1.2rem; transition: all 0.3s ease;
            display: flex; align-items: center; justify-content: center;
        }
        .btn-sil:hover { background: #ff6b6b; color: white; transform: scale(1.1); }

        .sepet-ozet {
            background: white; border-radius: 20px; padding: 2rem;
            box-shadow: 0 10px 40px rgba(0,0,0,0.06);
            margin-top: 2rem; animation: slideUp 0.6s ease forwards;
        }
        [data-theme="dark"] .sepet-ozet { background: rgba(40, 40, 60, 0.9); border: 1px solid var(--light-gray); }
        .sepet-ozet h3 { font-family: 'Cormorant Garamond', serif; font-size: 1.8rem; margin-bottom: 1.5rem; color: var(--charcoal); }
        .ozet-satir { display: flex; justify-content: space-between; margin-bottom: 1rem; color: var(--warm-gray); }
        .ozet-satir.toplam { border-top: 1px solid var(--light-gray); padding-top: 1rem; margin-top: 1rem; color: var(--charcoal); font-weight: 600; font-size: 1.2rem; }
        .ozet-tutar { color: var(--rose-deep); }
        .btn-satin-al {
        ...
        .btn-satin-al:hover { transform: translateY(-3px); box-shadow: 0 10px 30px rgba(232, 165, 184, 0.4); }

        /* Mobil Düzenlemeler - Sepet Sayfası İçin */
        @media (max-width: 768px) {
            .page-hero { padding: 8rem 5% 2rem; }
            .page-title { font-size: 2.5rem; }
            .sepet-section { padding: 1rem 5% 4rem; }
            .sepet-item {
            ...
            .sepet-resim { width: 80px; height: 80px; grid-row: span 2; }
            .sepet-adet-kontrol { grid-column: 1 / span 2; justify-content: flex-start; margin-top: 0.5rem; }
            .sepet-toplam { grid-column: 1 / span 2; text-align: left; font-size: 1.2rem; margin-top: 0.5rem; }
            .btn-sil { position: absolute; top: 1rem; right: 1rem; width: 32px; height: 32px; font-size: 1rem; }
            
            footer { padding: 3rem 5%; }
            .footer-quote { font-size: 1.2rem; }
        }

        footer { padding: 4rem 10%; text-align: center; background: rgba(255,255,255,0.3); backdrop-filter: blur(10px); }
        [data-theme="dark"] footer { background: rgba(0,0,0,0.2); }
        .footer-quote { font-family: 'Cormorant Garamond', serif; font-size: 1.5rem; font-style: italic; color: var(--rose-deep); margin-bottom: 1rem; }
    </style>
</head>
<body>
    ...
    <header class="page-hero">
        <div class="hero-content">
            <span class="page-greeting">Alışverişin Keyfini Çıkarın</span>
            <h1 class="page-title">Sepetim <span>Detayları</span></h1>
            <p class="page-subtitle">Seçtiğiniz ürünleri buradan yönetebilir, siparişinizi tamamlayabilirsiniz.</p>
        </div>
    </header>

    <section class="sepet-section">
        <div id="sepetListesi"></div>

        <div class="sepet-ozet" id="sepetOzet" style="display:none;">
            <h3>Sipariş Özeti</h3>
            ...
            <div class="ozet-satir">
                <span>Kargo</span>
                <span style="color:var(--sage)">Ücretsiz</span>
            </div>
            <div class="ozet-satir toplam">
                <span>Toplam</span>
                <span class="ozet-tutar" id="sepetToplam">0 ₺</span>
            </div>
            <button id="satinAlBtn" class="btn-satin-al">🛍 Satın Al</button>
        </div>
    </section>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', async () => {
            try {
                ...
                if (container) {
                    UrunlerApp.sepetRender(container);
                }

                const sepet = UrunlerApp.sepetiGetir();
                ...
                if (araToplam) araToplam.textContent = UrunlerApp.sepetToplamTutar().toLocaleString('tr-TR') + ' ₺';
                if (sepetToplam) sepetToplam.textContent = UrunlerApp.sepetToplamTutar().toLocaleString('tr-TR') + ' ₺';
                ...
                if (satinAlBtn) {
                    satinAlBtn.addEventListener('click', (e) => {
                        ...
                        <?php else: ?>
                            alert('Satın alma işlemi için önce giriş yapmanız gerekiyor. Yönlendiriliyorsunuz...');
                            window.location.href = 'giris_kayit.php';
                        <?php endif; ?>
                    });
                }
